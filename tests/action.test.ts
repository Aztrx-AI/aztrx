...
test(
  ...
  () => {
    ...
    try {
      ...
      // Asserted against VERSION, not a literal: this test exists to catch the
      // fallback drifting from package.json, so a hardcoded number here would
      // fail on every legitimate release — training the reader to bump it
      // blindly, which is exactly how the drift it guards against gets through.
      assert.equal(sh(snippet, { ACTION_PATH: "" }).stdout.trim(), VERSION, "must fall back when ACTION_PATH is empty");
      ...
      try {
        assert.equal(sh(snippet, { ACTION_PATH: bare }).stdout.trim(), VERSION);
      } finally {
